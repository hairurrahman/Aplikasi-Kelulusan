...
export default function SiswaPage() {
  ...
  const mataPelajaran = nilai?.mapel || {};
      ...
      <div style={{ position: "relative", zIndex: 1, padding: "0 0 40px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              {schoolInfo.logo && (
                <img src={schoolInfo.logo} alt="Logo"
                  style={{ width: 36, height: 36, borderRadius: "50%", objectFit: "cover", border: "2px solid rgba(255,255,255,0.5)" }} />
              )}
              <div>
        ...
        <div style={{ padding: "0 20px" }}>
          <div className="card animate-popIn" style={{
            ...
            <div style={{ fontSize: 72, marginBottom: 8 }} className="animate-bounce">
              {isLulus ? "🎉" : "😔"}
            </div>
            <h1 style={{
              fontSize: 32, marginBottom: 4,
              color: isLulus ? "#2ECC71" : "#E74C3C",
            }}>
              {isLulus ? "LULUS!" : "BELUM LULUS"}
            </h1>
            {isLulus && (
              <p style={{ fontSize: 14, color: "var(--text-light)", fontWeight: 700 }}>
          ...
          <div className="card animate-fadeIn" style={{ marginBottom: 16 }}>
            <h3 style={{ fontSize: 16, color: "var(--primary)", marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}>
              👤 Data Siswa
            </h3>
            {[
              ...
              <div key={i} style={{
                ...
                padding: "8px 0",
                borderBottom: i < 4 ? "1.5px dashed var(--border)" : "none",
              }}>
                <span style={{ fontSize: 13, color: "var(--text-light)", fontWeight: 600, display: "flex", gap: 6 }}>
                  <span>{item.icon}</span>{item.label}
                </span>
                <span style={{ fontSize: 13, fontWeight: 800, color: "var(--text)", textAlign: "right", maxWidth: "55%" }}>
                  {item.value}
                </span>
              </div>
            ))}
          ...
          {/* Nilai */}
          {nilai && Object.keys(mataPelajaran).length > 0 && (
            <div className="card animate-fadeIn" style={{ marginBottom: 16 }}>
              <h3 style={{ fontSize: 16, color: "var(--secondary)", marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}>
                📊 Nilai Kelulusan
              </h3>
              {Object.entries(mataPelajaran).map(([mapel, val], i) => (
                <div key={i} style={{
                  display: "flex", justifyContent: "space-between", alignItems: "center",
                  padding: "8px 0",
                  borderBottom: i < Object.entries(mataPelajaran).length - 1 ? "1.5px dashed var(--border)" : "none",
                }}>
                  <span style={{ fontSize: 13, color: "var(--text)", fontWeight: 700 }}>{mapel}</span>
                  <div style={{
                    background: val >= 70
                      ? "linear-gradient(135deg, var(--success), #3ECC61)"
          ...
          {dokumen?.url && (
            <div className="card animate-fadeIn" style={{
              ...
              <p style={{ fontSize: 13, color: "var(--text-light)", fontWeight: 600, marginBottom: 14 }}>
                SKL kamu sudah tersedia dan bisa diunduh!
              </p>
          ...
          <div className="card animate-fadeIn" style={{
            ...
            <p style={{ color: "white", fontWeight: 700, fontSize: 14, lineHeight: 1.6 }}>
              {isLulus
                ? "Selamat! Kamu telah menyelesaikan pendidikan dasar dengan baik. Semangat terus meraih impian! 🌟"
                : "Jangan menyerah! Setiap usaha pasti ada hasilnya. Terus belajar dan berkembang! 💪"}
            </p>
          </div>
        </div>
      </div>
